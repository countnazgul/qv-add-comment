...
function extension_Done() {
  Qva.AddExtension('Comments', function () {
    ...
    var QVobjectId = _this.Layout.Text5.text.toString();

    osUser = _this.Layout.Text6.text.toString();
    ...
    window.addComment = function () {
      ...
      if (comment.length > 0) {
        ...
        if (QVobjectId.length > 0) {

          var doc = Qv.GetCurrentDocument();
          var myObj = doc.GetObject(QVobjectId, function () {
            ...
            for (var i = 0; i < text.length; i++) {
              ...
              var field = t.substr(0, t.indexOf('^'));
              var values = t.substr(t.indexOf('^') + 1);
              var v = { field: field, value: values }
              result.push(v);
            }
    ...
    //$('#' + divName).append('<input type="button" id="test" value="test" onclick="test()"/>');
